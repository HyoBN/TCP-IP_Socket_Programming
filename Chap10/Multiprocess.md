...
#### 시그널과 signal 함수
 프로세스는 자식 프로세스의 종료라는 상황 발생시, 특정 함수의 호출을 운영체제에게 요구한다.
...
```
 ...
 - 역할 : 첫 번째 인자(signo)에 해당하는 상황이 발생하면 두 번째 인자로 전달된 함수를 호출한다.
 	 ex) signal(SIGCHLD, mychild); --> 자식프로세스가 종료되면(SIGCHLD) mychild 함수를 호출!
...
#### sigaction 함수.
 ...
 > 안전한 이유는 signal 함수는 UNIX 계열의 운영체제 별로 동작방식이 조금 차이가 날 수 있지만 sigaction함수는 그렇지 않다는 것이다.
...
둘 이상의 클라이언트에게 서비스를 제공하는 형태로 에코 서버를 확장.

방법 : 클라이언트의 연결 요청이 있을 때마다 에코 서버가 자식 프로세스를 생성해서 서버를 제공한다.
이를 위해서 다음의 과정이 필요하다.
1. 에코 서버(부모 프로세스)는 accept 함수호출을 통해 클라이언트들의 연결요청을 수락한다.
2. 이때 얻게 되는 소켓의 파일 디스크립터를 자식 프로세스를 생성해서 넘겨준다.
	- 자식프로세스는 부모의 파일 디스크립터까지도 복사한다.
